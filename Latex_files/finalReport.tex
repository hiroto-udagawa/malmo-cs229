...
Algorithm \ref{alg:algo1} summarizes the pipeline for Deep Q-learning and other important aspects such as experience replay and $\epsilon$-greedy strategy.
...
Behaviorally, the agent started each training run by moving and spinning arbitrarily, but ultimately learned to recognize zombies in its field of view and face them to connect hits.
...
We succesfully trained an agent controlled by a Deep Q-Network to move around a room and kill zombies given only visual input and a reward structure.
